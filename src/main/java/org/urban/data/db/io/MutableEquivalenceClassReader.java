...
/**
 * Reader for equivalence class files. Returns an index of mutable equivalence
 * classes.
 * 
...
public class MutableEquivalenceClassReader extends EquivalenceClassReader<MutableEquivalenceClass> {
    ...
    public MutableEquivalenceClass equivalenceClassFromString(String[] tokens) {
        ...
        return new MutableEquivalenceClass(
                Integer.parseInt(tokens[0]),
                new HashIDSet(tokens[1].split(",")),
                ColumnElementHelper.fromStringArray(tokens[2])
        );
    }
}
